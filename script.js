...
async function loadMovies(searchTerm) {
    const URL = `https://www.omdbapi.com/?s=${searchTerm}&page=1&apikey=79f27e7a`; // Fixed URL with quotes
    ...
    movies.forEach(movie => {
        ...
        movieListItem.innerHTML = `
            ...
            <div class="search-item-info">
                <h3>${movie.Title}</h3>
                <p>${movie.Year}</p>
                <button class="add-to-wishlist" onclick="addToWishlist('${movie.imdbID}', '${movie.Title}', '${moviePoster}', event)">Add to Wishlist</button>
            </div>
        `;
...
function loadMovieDetails() {
    ...
    searchListMovies.forEach(movie => {
        movie.addEventListener('click', async () => {
            ...
            const result = await fetch(`https://www.omdbapi.com/?i=${movie.dataset.id}&apikey=79f27e7a`); // Fixed URL with quotes
...
function addToWishlist(movieID, movieTitle, posterUrl, event) {
    ...
    if (!wishlist.some(item => item.id === movieID)) {
        ...
        // Notify the user
        alert(`${movieTitle} has been added to your wishlist!`); // Fixed template literals with backticks
    } else {
